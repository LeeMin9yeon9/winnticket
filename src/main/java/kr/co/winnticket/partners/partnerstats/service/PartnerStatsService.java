...
public class PartnerStatsService {
    ...
    //파트너 통계 전체
    public PartnerStatsSummaryDto getSummary(String partnerId, LocalDate startDate, LocalDate endDate) {
        return mapper.getSummary(partnerId, startDate, endDate);
    }

    // 파트너 일매출
    public List<PartnerDailySalesDto> getDailySales(String partnerId, LocalDate startDate, LocalDate endDate) {
        return mapper.getDailySales(partnerId, startDate, endDate);
    }

    // 파트너 카테고리 별 매출
    public List<PartnerCategorySalesDto> getCategorySales(String partnerId, LocalDate startDate, LocalDate endDate) {
        return mapper.getCategorySales(partnerId, startDate, endDate);
    }

    // 파트너 TOP3
    public List<PartnerTopProductsDto> getTopProducts(String partnerId, LocalDate startDate, LocalDate endDate) {
